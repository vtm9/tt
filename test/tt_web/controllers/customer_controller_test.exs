defmodule TtWeb.CustomerControllerTest do
  use TtWeb.ConnCase
  ...
  def fixture(:customer) do
    {:ok, customer} = Bill.create_customer(@create_attrs)
    customer
  end
  ...
  describe "create customer" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      new_conn = get(conn, Routes.customer_path(conn, :show, id))
      assert html_response(new_conn, 200) =~ "Show Customer"
    end
  ...
  describe "update customer" do
    ...
    test "redirects when data is valid", %{conn: conn, customer: customer} do
      ...
      new_conn = get(conn, Routes.customer_path(conn, :show, customer))
